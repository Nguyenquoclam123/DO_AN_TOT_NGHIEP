...
export default function ManageQuestionsPage() {
    ...
    const confirmSaveAll = async () => {
        try {
            ...
            let activeSetId = params.setId as string;
            const updatedQuestions = [...questions];
            
            // 1. Save new questions and update existing ones
            for (let i = 0; i < updatedQuestions.length; i++) {
                ...
                if (q.isNew) {
                    const result = await questionBankService.addQuestion(activeSetId, payload);
                    // Update the local item with the real ID and remove isNew flag
                    updatedQuestions[i] = {
                        ...updatedQuestions[i],
                        id: result.id,
                        isNew: false
                    ...
                    if (result && result.questionSetId) {
                        activeSetId = result.questionSetId;
                    }
                } else {
                    const result = await questionBankService.updateQuestion(q.id, payload);
                    if (result && result.newSetId) {
                        activeSetId = result.newSetId;
                    }
                }
            }

            // Sync the state back to ensure no more "isNew" flags remain
            setQuestions(updatedQuestions);

            // 2. Update Set Details (Name, Description) using the final activeSetId
            await questionBankService.updateSet(activeSetId, {
                name: set.name,
                description: set.description
            });

            // 3. Update order using the final activeSetId
            const updatedSet = await questionBankService.getSetDetail(activeSetId);
            const questionIds = updatedSet.questions.map((q: any) => q.id);
            await questionBankService.reorderQuestions(activeSetId, questionIds);

            router.push(`/employer/question-bank/set/${activeSetId}`);
        } catch (error) {
            console.error("Failed to save all changes", error);
            alert("Failed to save all changes. Please check your data.");
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', padding: '40px 64px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    <button onClick={() => router.back()} style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#64748b', marginTop: '4px' }}>
                        <ArrowLeft size={24} />
                    </button>
                    <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '4px' }}>
                            <input
                                ...
                                style={{ fontSize: '24px', fontWeight: 800, color: '#0f172a', border: 'none', background: 'transparent', outline: 'none', padding: '4px 0', borderBottom: '2px solid transparent', transition: 'all 0.2s', width: '100%' }}
                                onFocus={(e) => e.currentTarget.style.borderBottomColor = '#5C9AFF'}
                        ...
                        <input
                            ...
                            style={{ fontSize: '14px', color: '#94a3b8', border: 'none', background: 'transparent', outline: 'none', width: '100%' }}
                        />
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    <button 
                        ...
                        disabled={isSaving}
                        style={{ backgroundColor: '#1e40af', color: 'white', padding: '12px 32px', borderRadius: '12px', border: 'none', fontWeight: 800, fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 8px 16px -4px rgba(30, 64, 175, 0.2)', opacity: isSaving ? 0.7 : 1 }}
                    >
                            ...
                            {questions.map((q, index) => (
                                <Draggable key={q.id} draggableId={q.id} index={index}>
                                    {(provided) => (
                                        ...
                                        >
                                            <div style={{ backgroundColor: 'white', borderRadius: '24px', border: q.isNew ? '2px dashed #1e40af' : '1px solid #f1f5f9', padding: '24px', display: 'flex', gap: '20px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                                                {/* Drag Handle */}
                                                            ...
                                                            {[
                                                                ...
                                                                <button
                                                                    ...
                                                                    style={{ 
                                                                        ...
                                                                        color: q.type === t.id ? '#1e40af' : '#94a3b8',
                                                                        transition: 'all 0.2s'
                                                                    }}
                                                                >
                                                                    {t.label}
                                                    ...
                                                    {(q.type === 'MULTIPLE_CHOICE' || q.type === 'CHECKBOX') && (
                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                                            {q.options?.map((opt: any, optIdx: number) => (
                                                                <div key={opt.id} style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                                                    <div 
                                                                        ...
                                                                        style={{ width: '20px', height: '20px', borderRadius: '6px', border: `2px solid ${opt.isCorrect ? '#059669' : '#cbd5e1'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backgroundColor: opt.isCorrect ? '#ecfdf5' : 'transparent', flexShrink: 0 }}
                                                                    >
                                                                        {opt.isCorrect && <CheckCircle2 size={14} color="#059669" />}
                                                                    </div>
                                                                    <input
                                                            ...
                                                            <button 
                                                                ...
                                                                style={{ border: 'none', background: 'none', color: '#1e40af', fontSize: '11px', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', width: 'fit-content', marginTop: '4px' }}
                                                            >
            ...
            >
                <Plus size={24} /> Add Question at the End
            </button>
            ...
            <ConfirmModal 
                ...
                title="Xác nhận xóa"
                message="Bạn có chắc chắn muốn xóa câu hỏi này không? Thao tác này không thể hoàn tác."
            ...
            <ConfirmModal 
                ...
                title="Lưu thay đổi"
                message="Lưu lại toàn bộ các câu hỏi đã chỉnh sửa vào ngân hàng câu hỏi của công ty?"
